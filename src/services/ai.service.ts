...
async function getYouTubeSummary(videoUrl: string): Promise<string> {
  ...
  const result = await model.generateContent(
    `Please provide a detailed summary of the following content, including all key points and relevant details, in a human-readable format:\n\n${textToSummarize}`
  );
...
async function getGitHubSummary(repoUrl: string): Promise<string> {
  ...
  const textToSummarize = `${response.data.description} ${Buffer.from(
